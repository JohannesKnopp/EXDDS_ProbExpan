...
if __name__ == '__main__':
    ...
    parser.add_argument('-vocab', default='intermediate/entity2id.txt', help='vocab file')
    ...
    if args.pretrained_model is not None:
        ...
        if args.ensemble:
            for i in model_ids:
                print('[Model %d]' % i)
                if args.CL:
                    expan.pretrain(args.save_path+str(i), lr=1.5e-5, epoch=1, batchsize=256, num_sen_per_entity=256, smoothing=0.075)
                else:
                    expan.pretrain(args.save_path+str(i), lr=1e-5, epoch=1, batchsize=128, num_sen_per_entity=256, smoothing=0.075)
                print('')
            exit(0)
        else:
            if args.CL:
                expan.pretrain(args.save_path, lr=1.5e-5, epoch=1, batchsize=256, num_sen_per_entity=256, smoothing=0.075)
            else:
                expan.pretrain(args.save_path, lr=1e-5, epoch=1, batchsize=128, num_sen_per_entity=256, smoothing=0.075)
            exit(0)
    if args.ensemble:
            ...
            for j, cls in enumerate(class_names):
                with open(os.path.join(args.dataset, args.output, f'{i}_{cls}'), 'w') as f:
                    ...
                    for eid in expanded[j]:
                        print(f'{eid}\t{expan.eid2name[eid].encode("utf-8")}', file=f)
